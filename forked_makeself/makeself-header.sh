...
ownership=y
verbose=n
headerargsviaenv="y"  # nominaly off , probably controlable from flags in makeself

if test x"$headerargsviaenv" = xy; then
	set -- $MS_HEADER_ARGS -- $@
fi
...
# gulfie
# fixup targetdir if needed. 
# 

if test x"\$targetdirrelativetoarchive" = xy; then 
	# 
	RUNCMD="\$0"
	tmpdirname=\`dirname "\$0"\`
	
	if test x"\$tmpdirname" = x; then
		tmpdirname="."
	fi
	targetdir="\$tmpdirname/\$targetdir"
fi
